//! Guarded A/B benchmark for borrowed replication-frame decoding.
...
fn main() {
    ...
    let path_ok = match config.mode {
        DecodeMode::Borrowed => stats.owned_frame_materializations == 0,
        DecodeMode::Owned => stats.owned_frame_materializations == expected_frames,
    };
    ...
    println!("SectorSync borrowed replication decode benchmark");
    ...
    println!("borrowed_decode={}", config.mode == DecodeMode::Borrowed);
    println!("frames={}", stats.frames);
    println!("decoded_entities={}", stats.entities);
    println!("decoded_components={}", stats.components);
    println!("decoded_payload_bytes={}", stats.payload_bytes);
    ...
    println!("threshold_decode_path_ok={path_ok}");
...
fn run(config: Config) -> Stats {
    ...
    'ticks: for _ in 0..config.ticks {
        ...
        for _ in 0..config.frames_per_tick {
            ...
            match config.mode {
                DecodeMode::Borrowed => consume_borrowed(&mut decoder, &wire, &mut stats),
                DecodeMode::Owned => consume_owned(&mut decoder, &wire, &mut stats),
            }
...
fn consume_borrowed(decoder: &mut BinaryFrameDecoder, wire: &[u8], stats: &mut Stats) {
    ...
    for entity in frame.entities() {
        ...
        for component in entity.components() {
            consume_component(component.bytes, stats);
        }
    }
    black_box(frame);
}

fn consume_owned(decoder: &mut BinaryFrameDecoder, wire: &[u8], stats: &mut Stats) {
    let RuntimeFrame::Replication(frame) = decoder
        .decode(wire)
        .expect("guarded owned frame should decode")
    else {
        unreachable!("benchmark wire is replication");
    };
    stats.owned_frame_materializations = stats.owned_frame_materializations.saturating_add(1);
    for entity in &frame.entities {
        stats.entities = stats.entities.saturating_add(1);
        for component in &entity.components {
            consume_component(&component.bytes, stats);
...
fn encoded_frame(config: Config) -> Vec<u8> {
